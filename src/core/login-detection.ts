import type { AuthState, LoginChecks } from "../shared/types";
...
const hasStrongLoginCookie = (cookieNames: string[]): boolean =>
  cookieNames.some((cookieName) => STRONG_LOGIN_COOKIE_PATTERN.test(cookieName));

export const detectLoginStateFromProbe = (probe: LoginProbe): AuthState => {
  ...
  if (
    CLOUDFLARE_PATTERN.test(probe.bodyText) ||
...
export const canProceedAfterManualConfirmation = (checks: LoginChecks): boolean => {
  if (checks.linuxDo === "LOGGED_IN" && checks.cdkLinuxDo === "LOGGED_IN") {
    return true;
  }

  const hasBlockingState =
    checks.linuxDo === "LOGIN_NEEDED" ||
