type AuthState = "LOGGED_IN" | "LOGIN_NEEDED" | "CLOUDFLARE_CHALLENGE" | "UNKNOWN";
type CdkStatus = "NO_CDK" | "WAITING" | "CLAIMABLE" | "CLAIMING" | "CLAIMED" | "OUT_OF_STOCK" | "FAILED" | "ENDED" | "LOGIN_REQUIRED";
type MonitorPhase =
  | "STARTUP_LOGIN_CHECK"
...
const formatTime = (value: string | null): string => (value ? formatter.format(new Date(value)) : "-");
const formatKeywords = (keywords: string[]): string => keywords.join(", ");

const getAuthLabel = (authState: AuthState): string => {
  switch (authState) {
    case "LOGGED_IN":
      return "已登录";
    case "LOGIN_NEEDED":
...
const getLoginCheckLabel = (authState: AuthState): string => {
  switch (authState) {
    case "LOGGED_IN":
      return "已登录";
    case "LOGIN_NEEDED":
...
const getPhaseLabel = (phase: MonitorPhase): string => {
  switch (phase) {
...
const getStatusClass = (status: CdkStatus): string => {
  switch (status) {
    case "CLAIMED":
      return "chip-success";
    case "WAITING":
...
const getStatusLabel = (status: CdkStatus): string => {
  switch (status) {
    case "NO_CDK":
      return "无需领取";
    case "WAITING":
...
const renderLinks = (row: TopicRow): string => {
...
const bindEvents = (): void => {
  ...
  document.querySelector<HTMLButtonElement>("#confirm-login")?.addEventListener("click", () => {
    void window.monitorApi.confirmLogin();
  });

  document.querySelector<HTMLButtonElement>("#reinit-browser")?.addEventListener("click", () => {
    void window.monitorApi.reinitializeBrowser();
  });

  document.querySelector<HTMLButtonElement>("#open-debug-dir")?.addEventListener("click", () => {
    void window.monitorApi.openDebugDirectory();
  });

  document.querySelector<HTMLButtonElement>("#open-debug-log")?.addEventListener("click", () => {
    void window.monitorApi.openDebugLog();
  });

  document.querySelector<HTMLButtonElement>("#save-settings")?.addEventListener("click", async () => {
    const scanInterval = Number((document.querySelector<HTMLInputElement>("#scan-interval")?.value ?? "30").trim());
    const keywords = (document.querySelector<HTMLInputElement>("#keywords")?.value ?? "")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean);
    const notificationsEnabled = document.querySelector<HTMLInputElement>("#notifications")?.checked ?? true;
    ...
    currentSnapshot = snapshot as Snapshot;
    render(currentSnapshot);
  });

  document.querySelectorAll<HTMLButtonElement>("[data-topic-id]").forEach((element) => {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-cdk-id]").forEach((element) => {
    element.addEventListener("click", () => {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-retry-id]").forEach((element) => {
    element.addEventListener("click", () => {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-external]").forEach((element) => {
    element.addEventListener("click", () => {
...
const bootstrap = async (): Promise<void> => {
  const snapshot = (await window.monitorApi.getSnapshot()) as Snapshot;
  currentSnapshot = snapshot;
  render(snapshot);
  window.monitorApi.onSnapshot((nextSnapshot: Snapshot) => {
