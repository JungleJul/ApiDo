...
import { getClaimableManualMessage, getManualClaimAlertPlan } from "../core/cdk-alerts";
import { describeCdkDecisionBasis, inspectCdkTextCandidates, shouldRetryCdkTextInspection } from "../core/cdk-debug";
import { analyzeCdkPageText } from "../core/cdk-page";
import { classifyCdkWindow, parseCdkWindow } from "../core/cdk-time";
import { DebugRuntime } from "../core/debug-runtime";
import { buildEdgeCdpLaunchPlan, resolveEdgeCdpWebSocketUrl, resolveEdgeExecutablePath } from "../core/edge-cdp";
import { canProceedAfterManualConfirmation, detectLoginStateFromProbe } from "../core/login-detection";
import { describeLoginGate, describeScanOutcome, isBrowserSessionUsable, type MonitorPhase } from "../core/monitor-progress";
import { SettingsStore } from "../core/settings-store";
import { createCloudflareBackoffState, getEffectiveScanIntervalSeconds, shouldSkipLatestScan } from "../core/scan-guard";
import { sortTopicRows } from "../core/topic-row-sort";
import { shouldDeepReadTopic } from "../core/topic-cooldown";
import { SqliteRepository } from "../core/repository";
import { isTopicNew, delay } from "../core/time";
import { selectCandidateTopics } from "../core/topic-rules";
import { collectLinks } from "../core/urls";
...
const LOGIN_NEEDED_STATES: AuthState[] = ["LOGIN_NEEDED", "CLOUDFLARE_CHALLENGE"];
...
type EnqueueOutcome = "ENQUEUED" | "COOLDOWN" | "DUPLICATE";

const createInitialProgress = (): RuntimeProgress => ({
...
export class MonitorService extends EventEmitter {
  ...
  private cloudflarePauseUntil: string | null = null;
  private consecutiveCloudflareCount = 0;
  private scanInterval: NodeJS.Timeout | null = null;
  private loginPollInterval: NodeJS.Timeout | null = null;
  private browserContext: BrowserContext | null = null;
  private browserConnection: Browser | null = null;
  private edgeProcess: ChildProcess | null = null;
  private edgeRemotePort: number | null = null;
  private queue: CandidateTopic[] = [];
  private queuedTopicIds = new Set<number>();
  private queueRunning = false;
  private lastTopicReadAt = 0;
  private activeScanPromise: Promise<void> | null = null;
  private readonly scheduledClaimChecks = new Map<number, NodeJS.Timeout>();
  private readonly scheduledPreAlerts = new Map<number, NodeJS.Timeout>();
  private readonly scheduledExternalCdkChecks = new Map<number, NodeJS.Timeout>();
  private readonly preAlertNotified = new Set<number>();
  private readonly claimableNotified = new Set<number>();
  private progress: RuntimeProgress = createInitialProgress();
  ...
  getSnapshot(): AppStatusSnapshot {
    const rows = sortTopicRows(this.repository.listRows());
    return {
  ...
  async updateSettings(partial: Partial<AppConfig>): Promise<AppStatusSnapshot> {
  ...
  async triggerRescan(): Promise<void> {
    ...
    const gate = describeLoginGate(this.progress.loginChecks);
  ...
  async retryClaim(topicId: number): Promise<void> {
  ...
  async confirmLogin(): Promise<void> {
    ...
    if (fullyLoggedIn || canProceedAfterManualConfirmation(checks)) {
      ...
      this.setProgress(
        "READY",
        fullyLoggedIn ? "登录状态已确认，程序可以开始扫描。" : "已按你的手动确认继续，后续会在实际访问页面时再次校验登录态。",
        this.getNextScanAt()
      );
      this.emitSnapshot();
      await this.triggerRescan();
  ...
  async reinitializeBrowser(): Promise<void> {
  ...
  async noteCdkOpenedExternally(topicId: number): Promise<void> {
  ...
  getTopicUrl(topicId: number): string | null {
    return this.repository.getTopic(topicId)?.normalizedTopicUrl ?? null;
  }

  getCdkUrl(topicId: number): string | null {
    return this.repository.getCdk(topicId)?.cdkUrl ?? null;
  }

  private buildProgressSnapshot(): RuntimeProgress {
    return {
      ...this.progress,
  ...
  private setProgress(phase: MonitorPhase, detail: string, nextActionAt: string | null = null): void {
    this.progress = {
      ...this.progress,
      phase,
      detail,
  ...
  private getNextScanAt(): string {
    return new Date(Date.now() + getEffectiveScanIntervalSeconds(this.config.scanIntervalSeconds) * 1000).toISOString();
  }

  private async ensureLoginReady(notifyUser: boolean): Promise<boolean> {
    this.setProgress("STARTUP_LOGIN_CHECK", "正在检查 linux.do 和 cdk.linux.do 的登录状态。", null);
    this.emitSnapshot();

    const checks = await this.refreshLoginChecks();
    const gate = describeLoginGate(checks);
  ...
  private async refreshLoginChecks(): Promise<LoginChecks> {
  ...
  private async inspectLoginState(url: string, artifactLabel: string): Promise<AuthState> {
    ...
    const pageMarkers = await page
      .evaluate(() => ({
        ...
        hasLoginForm: Boolean(
          document.querySelector('form[action*="login"], form[action*="session"], button[type="submit"], .login-button, .btn-primary')
        ),
        hasLogoutAction: Boolean(document.querySelector('a[href*="logout"], button[data-logout], .logout, .sign-out')),
        hasCurrentUserMarker: Boolean(
          document.querySelector('.current-user, .user-menu, a[href*="/u/"], img.avatar, .header-dropdown-toggle, .user-nav')
        )
      }))
      .catch(() => ({
    ...
    const detectedState = detectLoginStateFromProbe({
      currentUrl,
      bodyText,
      pageTitle,
      cookieNames,
  ...
  private async openLoginPages(checks: LoginChecks): Promise<void> {
  ...
  private isBrowserSessionHealthy(): boolean {
    const basicHealth = isBrowserSessionUsable({
  ...
  private restartScanLoop(): void {
    if (this.scanInterval) {
      clearInterval(this.scanInterval);
    }

    const effectiveIntervalSeconds = getEffectiveScanIntervalSeconds(this.config.scanIntervalSeconds);
  ...
  private async scanLatest(): Promise<void> {
    const now = new Date();
    if (shouldSkipLatestScan(this.cloudflarePauseUntil, now)) {
      this.setProgress(
        "CLOUDFLARE_BACKOFF",
        `Cloudflare 退避中，将在 ${new Date(this.cloudflarePauseUntil ?? now.toISOString()).toLocaleString("zh-CN", {
          hour12: false,
          timeZone: APP_TIMEZONE
        })} 后再次尝试。`,
        this.cloudflarePauseUntil
      );
      this.emitSnapshot();
      return;
    }
    ...
    try {
      ...
      if (this.authState === "CLOUDFLARE_CHALLENGE") {
        const backoff = createCloudflareBackoffState(this.consecutiveCloudflareCount, now);
        this.consecutiveCloudflareCount = backoff.consecutiveCloudflareCount;
        this.cloudflarePauseUntil = backoff.pauseUntil;
        this.lastError = `检测到 Cloudflare 验证，自动扫描已暂停到 ${new Date(backoff.pauseUntil).toLocaleString("zh-CN", { hour12: false, timeZone: APP_TIMEZONE })}。`;
      ...
      const candidates = selectCandidateTopics(listItems, this.config.keywords);
      let enqueuedCount = 0;
      ...
      const summary = describeScanOutcome({
        listItemCount: listItems.length,
        candidateCount: candidates.length,
        enqueuedCount,
        cooldownSkippedCount,
  ...
  private enqueueCandidate(candidate: CandidateTopic): EnqueueOutcome {
    ...
    if (existing && !shouldDeepReadTopic(existing.lastDeepReadAt, this.config.topicCooldownMinutes, new Date())) {
  ...
  private async deepReadTopic(candidate: CandidateTopic): Promise<void> {
    try {
      const page = await this.getWorkingPage();
  ...
  private async handleCdk(topicId: number, cdkUrl: string): Promise<void> {
    try {
      const page = await this.getWorkingPage();
      ...
      const parsedWindow = parseCdkWindow(bodyText);

      if (parsedWindow) {
        const windowStatus = classifyCdkWindow(parsedWindow, new Date());
        ...
        if (windowStatus === "ENDED") {
          ...
          this.repository.upsertCdk({
            ...
            lastDecisionBasis: describeCdkDecisionBasis({
        ...
        const alertPlan = getManualClaimAlertPlan(parsedWindow, new Date());
      ...
      const record = this.evaluateManualClaimState(
        topicId,
        cdkUrl,
        parsedWindow?.startAt ?? null,
        parsedWindow?.endAt ?? null,
        bodyText,
        textInspection.source,
        textInspection.summary,
        textInspection.looksLikeHydration,
        textInspection.retryAttempts
      );
  ...
  private evaluateManualClaimState(
    topicId: number,
    cdkUrl: string,
    startAt: string | null,
    endAt: string | null,
    ...
    this.clearCdkTimers(topicId);
    const analysis = analyzeCdkPageText(bodyText);
    ...
    if (analysis.status === "CLAIMABLE") {
      const message = analysis.message || getClaimableManualMessage();
  ...
  private async collectCdkTextInspection(page: Page, topicId: number, cdkUrl: string) {
    let latestInspection = inspectCdkTextCandidates([{ source: "title", text: (await page.title().catch(() => "")) ?? "" }]);
    ...
    for (let attempt = 1; attempt <= CDK_TEXT_READ_RETRY_COUNT; attempt += 1) {
      ...
      latestInspection = inspectCdkTextCandidates([
      ...
      if (!shouldRetryCdkTextInspection(latestInspection) || attempt === CDK_TEXT_READ_RETRY_COUNT) {
        return { ...latestInspection, retryAttempts: attempt };
      }

      await delay(CDK_TEXT_READ_RETRY_DELAY_MS);
    }

    return latestInspection;
  }

  private restoreWaitingClaims(): void {
  ...
  private scheduleManualClaimNotifications(
    topicId: number,
    startAt: string,
  ...
  private clearCdkTimers(topicId: number): void {
    const preAlertTimer = this.scheduledPreAlerts.get(topicId);
    if (preAlertTimer) {
      clearTimeout(preAlertTimer);
      this.scheduledPreAlerts.delete(topicId);
  ...
  private async checkManualLoginProgress(): Promise<void> {
  ...
  private async extractTopicList(page: Page): Promise<TopicListItem[]> {
    const rows = await page.evaluate((selector) => {
      const nodes = Array.from(document.querySelectorAll(selector)).slice(0, 10);
      return nodes
        .map((row) => {
          const titleElement = row.querySelector<HTMLAnchorElement>("a.title, .title a, a.raw-topic-link");
          const tagElements = Array.from(row.querySelectorAll<HTMLElement>(".discourse-tag, .simple-topic-list__tag, .badge-category__name"));
          return {
  ...
  private async extractTopicSnapshot(page: Page, fallbackTitle: string): Promise<TopicSnapshot> {
    const raw = await page.evaluate((titleBackup) => {
      const title = document.querySelector<HTMLElement>("#topic-title .fancy-title, h1.fancy-title, h1")?.textContent?.trim() ?? titleBackup;
      const publishedAt = document.querySelector<HTMLTimeElement>("article#post_1 time[datetime], #post_1 time[datetime], .topic-post:first-child time[datetime]")?.dateTime ?? null;
      const links = Array.from(document.querySelectorAll<HTMLAnchorElement>("article#post_1 .cooked a[href], #post_1 .cooked a[href], .topic-post:first-child .cooked a[href]"))
        .map((node) => node.href)
        .filter(Boolean);
    ...
    const { cdkLinks, externalLinks } = collectLinks(raw.links);
    return {
      topicId: 0,
      title: raw.title,
      topicUrl: page.url(),
      ...
      externalLinks,
      cdkLinks,
      isNew: isTopicNew(raw.publishedAt)
    };
  }
  ...
  private async relaunchContext(headless: boolean): Promise<void> {
    await this.closeBrowserResources();
    ...
    this.browserContext = await chromium.launchPersistentContext(path.join(this.userDataDir, "playwright-profile"), {
  ...
  private async launchEdgeViaCdp(): Promise<void> {
    const executablePath = resolveEdgeExecutablePath(this.debugRuntime.config.edgeExecutablePath);
    const remoteDebuggingPort = await this.getFreePort();
    const launchPlan = buildEdgeCdpLaunchPlan({
      executablePath,
      remoteDebuggingPort,
      userDataDir: path.join(this.userDataDir, "edge-profile"),
      startUrl: "about:blank"
    });

    this.debugRuntime.info("browser", "Launching native Edge instance for CDP attach", launchPlan);
    this.edgeProcess = spawn(launchPlan.executablePath, launchPlan.args, {
      stdio: "ignore",
      windowsHide: false
    });
    ...
    this.browserConnection = await chromium.connectOverCDP(wsEndpoint);
  ...
  private async waitForEdgeCdpReady(port: number, timeoutMs: number): Promise<string> {
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (response.ok) {
          ...
          return resolveEdgeCdpWebSocketUrl(payload);
        }
      } catch {
      }
  ...
  private async detectAuthState(page: Page): Promise<AuthState> {
    const bodyText = ((await page.textContent("body").catch(() => "")) ?? "").slice(0, 8000);
    const topicRows = await page.locator(TOPIC_LIST_SELECTOR).count().catch(() => 0);

    let nextState: AuthState = "UNKNOWN";
